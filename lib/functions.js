...
const wait = async (media) => new Promise(async (resolve, reject) => {
    ...
    try {
    	...
    	let belief = () => similarity < 0.89 ? "Tengo poca confianza en esto : " : ""
...
const simih = async (text) => {
	try {
		...
		return 'Simi no sabe de lo que estás hablando, ¿es un idioma extraño, hermana?'
	}
}
...
const start = (id, text) => {
	spins.add(id, {text: text})
		/*setTimeout(() => {
			spins.succeed('load-spin', {text: 'éxito'})
		}, Number(wait) * 1000)*/
	}
const info = (id, text) => {
...
const banner = cfonts.render(('Wamy!Bot|Wamy!Bot'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
